...
function getTitle(event) {
  ...
  if (STATUS_UPDATE_EVENTS.includes(event.meta.event)) {
    if (event.event_type) {
      title = `${event.meta.event}: ${event.event_type}`;
    } else if (event.status) {
      title = `${event.meta.event}: ${event.status}`;
    } else {
...
function Arrow({ selected, event, handleClick }) {
  ...
  return (
    <div className="arrow">
      <div className={`arrowTitle ${selected ? "selected" : null}`} onClick={handleClick}>
        {getTitle(event)}
      </div>
...
function App() {
  ...
  useEffect(() => {
    if (events) {
      const originConveyoEvents = events.filter(
        (value) => ORIGIN_CONVEYO_EVENTS.includes(value.meta.event) || isTransmissionBackendProcess(value)
      );
      setOriginConveyoEvents(originConveyoEvents);
      setOrderSendEvent(originConveyoEvents[0]);
      setConveyoTargetEvents(events.filter((value) => CONVEYO_TARGET_EVENTS.includes(value.meta.event)));
      ...
      <div className="rootContainer">
        <div className="header">
          ...
          <div className="inputContainer">
                ...
                >
                  <optgroup label="Staging">
                    {stagingList.map((value, index) => {
                      return (
                        <option key={index} value={`YGY-staging-${value}`}>
                          YGY-staging-{value}
                        </option>
                      );
                    })}
            ...
            <div className="inputRightSection">
              ...
              <button className="requestButton" onClick={getHubyoData}>
                Send Request
              </button>
            </div>
          </div>
        ...
        {events ? (
          <div className="componentsDisplay">
              ...
              <div className="orderyoBackendProcessContainer">
                ...
                {orderyoBackendProcessEvents.map((value, index) => {
                  return (
                    <AlimTalk
                      event={value}
                      selected={value.eventIndex === selectedEventIndex}
                      handleClick={() => handleEventClick(value, index, "R")}
                    />
                  );
                })}
              </div>
            ) : null}
            <div className="component">Origin {`${orderSendEvent ? `(${orderSendEvent.meta.publisher})` : ""}`}</div>
            ...
            <div className="component">Conveyo</div>
              ...
              {conveyoTargetEvents.map((value, index) => {
                return (
                  <Arrow
                    ...
                    selected={value.eventIndex === selectedEventIndex}
                    event={value}
                    handleClick={() => handleEventClick(value, index, "L")}
                  />
                );
              })}
            </div>

            <div className="component">Targets</div>
          </div>
        ) : null}
        {showEventLog ? (
          <div className={`eventViewContainer ${logPosition === "L" ? "onLeft" : "onRight"}`}>
            ...
            <div className="previewWrapper">
              <pre className="codePreview">{JSON.stringify(currentEvent, null, 2)}</pre>
            </div>
          </div>
        ) : null}
      </div>
